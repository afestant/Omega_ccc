...
#define TreeTrueRecoOmegaccc_cxx
...
#include "TreeTrueRecoOmegaccc.h"
#include "TreeReconstruction.h"

#endif
void DrawCorrelationPlots(TreeGenOmegaccc &t, TreeRecoTracks &t_rec, TreeGenV0Reco &t_rec_v0, TreeGenCascV0Reco &t_rec_casc, TreeCutVarOmegaccc &t_rec_omegaccc);
void DrawReconstructionPlots(TreeReconstruction &t);

void tree_analysis(){
    //TFile *fout = new TFile("tree_reco.root","RECREATE");
    TFile *f=TFile::Open("./AnalysisResults_V0CascTrees.root");
    ...
    TTree    *treeTrueRecoOmegaccc  = (TTree*)d->Get("TreeTrueRecoOmegaccc");
...
//}
    ...
    const int npart = 9;
    TString part[npart] = {"pi_from_l", "p_from_l", "k_from_o", "l", "o","omegaccc","pi_from_omegaccc","pi_from_omegacc","pi_from_omegac"};
    TH1F* hPt_gen[npart];
    ...
    float minx[npart]={0.,0.,0.,0.,0.,0.,0.,0.,0.};
    float maxx[npart]={10.,10.,10.,10.,10.,20.,20.,20.,20.};
    float miny[npart]={0.,0.,0.,0.,0.,0.,0.,0.,0.};
    float maxy[npart]={100.,100.,100.,100.,2.,0.1,0.5,0.5,0.5};
    int nbinsx[npart]={100,100,100,100,100,100,100,100,100};
    int nbinsy[npart]={1000,1000,1000,1000,1000,1000,1000,1000,1000};
    ...
    for(int k=0; k<npart; k++){
        TCanvas *c = new TCanvas(Form("c_%s",part[k].Data()),Form("c_%s",part[k].Data()),1440,830);
        c->Divide(3,2);
        c->cd(1);
        gPad->SetLogy();
        hPt_gen[k]->Draw("same");
    ...
    DrawCorrelationPlots(t_gen, t_rec, t_rec_v0, t_rec_casc, t_rec_omegaccc);
}
...
void DrawCorrelationPlots(TreeGenOmegaccc &t, TreeRecoTracks &t_rec, TreeGenV0Reco &t_rec_v0, TreeGenCascV0Reco &t_rec_casc, TreeCutVarOmegaccc &t_rec_omegaccc){
    ...
    TH2D *hNParticlesGen = new TH2D("hNParticlesGen","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
    TH2D *hNParticlesReconstructable = new TH2D("hNParticlesReconstructable","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
    TH2D *hNParticlesReconstructable2 = new TH2D("hNParticlesReconstructable2","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
    TH2D *hNParticlesReconstructableOverGen = new TH2D("hNParticlesReconstructableOverGen","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
    TH2D *hNParticlesReconstructed = new TH2D("hNParticlesReconstructed","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
    TH2D *hNParticlesReconstructedOverReconstructable = new TH2D("hNParticlesReconstructedOverReconstructable","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
    TH2D *hNParticlesReconstructedOverReconstructable2 = new TH2D("hNParticlesReconstructedOverReconstructable2","; ;pt",11,0.,11.,nbinspt,minpt,maxpt);
...
//    hNParticlesReconstructed->LabelsOption("v");
    ...
    for(int irecomegaccc = 0; irecomegaccc<nrecomegaccc; irecomegaccc++){
        t_rec_omegaccc.GetEntry(irecomegaccc);
        if(t_rec_omegaccc.fIsSignal==true)
    ...
    for(int ii=0; ii<npar; ii++){
        ...
        TCanvas *c=new TCanvas(Form("%s",particles[ii].Data()),Form("%s",particles[ii].Data()),1200,900);
        c->Divide(4,2);
        c->cd(1);
        hNPartGen[ii]->Draw();
        c->cd(2);
    ...
    for(int t=0; t<ncorr; t++){
        TCanvas *c=new TCanvas(Form("c_corr_%s",corr[t].Data()),Form("c_corr_%s",corr[t].Data()),1200,800);
        c->Divide(3,1);
        c->cd(1);
    ...
    for(int t=0; t<ncorrhits; t++){
        TCanvas *c=new TCanvas(Form("c_hits_correlation%s",partcorrnhits[t].Data()),Form("c_hits_correlation%s",partcorrnhits[t].Data()),1200,800);
        c->Divide(3,1);
        c->cd(1);
...
void DrawReconstructionPlots(TreeReconstruction &t){
    ...
    for(int i=0; i<2; i++){
        TString name = Form("%s",reco[i].Data());
        for(int g=0; g<npart; g++){
            name += Form("_%s",part[g].Data());
    ...
    for(int i=0; i<npart; i++){
        TCanvas *c_rxy= new TCanvas(Form("c_PtVsRxy_%s",part[i].Data()),Form("c_PtVsRxy_%s",part[i].Data()),1200,800);
        c_rxy->Divide(2,1);
        TCanvas *c_z= new TCanvas(Form("c_PtVsZ_%s",part[i].Data()),Form("c_PtVsZ_%s",part[i].Data()),1200,800);
        c_z->Divide(2,1);
        TCanvas *c_eta= new TCanvas(Form("c_PtVsEta_%s",part[i].Data()),Form("c_PtVsEta_%s",part[i].Data()),1200,800);
        c_eta->Divide(2,1);
        TCanvas *c_rap= new TCanvas(Form("c_PtVsRap_%s",part[i].Data()),Form("c_PtVsRap_%s",part[i].Data()),1200,800);
        c_rap->Divide(2,1);
        TCanvas *c_phi= new TCanvas(Form("c_PtVsPhi_%s",part[i].Data()),Form("c_PtVsPhi_%s",part[i].Data()),1200,800);
        c_phi->Divide(2,1);
        TCanvas *c_nreconstructable= new TCanvas(Form("c_PtVsNreconstructable_%s",part[i].Data()),Form("c_PtVsNreconstructable_%s",part[i].Data()),1200,800);
