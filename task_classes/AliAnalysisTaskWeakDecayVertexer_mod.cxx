...
ClassImp(AliAnalysisTaskWeakDecayVertexer_mod)

AliAnalysisTaskWeakDecayVertexer_mod::AliAnalysisTaskWeakDecayVertexer_mod()
...
AliAnalysisTaskWeakDecayVertexer_mod::AliAnalysisTaskWeakDecayVertexer_mod(const char *name, TString lExtraOptions)
...
//________________________________________________
{
    ...
    fV0VertexerSels[1] =   0.0;//0.02;  // min allowed impact parameter for the 1st daughter (LHC09a4 : 0.05)
    fV0VertexerSels[2] =   0.0;//0.02;  // min allowed impact parameter for the 2nd daughter (LHC09a4 : 0.05)
    fV0VertexerSels[3] =   3.0;//2.0 ;  // max allowed DCA between the daughter tracks       (LHC09a4 : 0.5)
    fV0VertexerSels[4] =   0.8;//0.95;  // min allowed cosine of V0's pointing angle         (LHC09a4 : 0.99)
    fV0VertexerSels[5] =   0.;//1.0 ;  // min radius of the fiducial volume                 (LHC09a4 : 0.2)
    fV0VertexerSels[6] =   300.;//200.  ;  // max radius of the fiducial volume                 (LHC09a4 : 100.0)
    ...
    fCascadeVertexerSels[3] =   0.01;//0.03 ;  // min allowed bachelor's impact parameter            (PDC07 : 0.035  / LHC09a4 : 0.025 )
    fCascadeVertexerSels[4] =   5.0;//2.0  ;  // max allowed DCA between the V0 and the bachelor    (PDC07 : 0.1    / LHC09a4 : 0.2   )
    fCascadeVertexerSels[5] =   0.8;//0.95 ;  // min allowed cosine of the cascade pointing angle   (PDC07 : 0.9985 / LHC09a4 : 0.998 )
    fCascadeVertexerSels[6] =   0.;//0.4  ;  // min radius of the fiducial volume                  (PDC07 : 0.9    / LHC09a4 : 0.2   )
    fCascadeVertexerSels[7] =   300.;//200.   ;  // max radius of the fiducial volume                  (PDC07 : 100    / LHC09a4 : 100   )
...
void AliAnalysisTaskWeakDecayVertexer_mod::UserCreateOutputObjects()
{
    //------------------------------------------------
...
//________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::UserExec(Option_t *)
{
    // Main loop
...
//________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::Terminate(Option_t *)
{
    ...
    if(!cRetrievedList) {
        Printf("ERROR - AliAnalysisTaskWeakDecayVertexer_mod : ouput data container list not available\n");
        return;
    }
    
    fHistEventCounter = dynamic_cast<TH1D*> (  cRetrievedList->FindObject("fHistEventCounter")  );
    if (!fHistEventCounter) {
        Printf("ERROR - AliAnalysisTaskWeakDecayVertexer_mod : fHistEventCounter not available");
        return;
    }
    
    TCanvas *canCheck = new TCanvas("AliAnalysisTaskWeakDecayVertexer_mod","V0 Multiplicity",10,10,510,510);
...
//________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::SetupStandardVertexing()
...
//________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::SetupLooseVertexing()
//Meant to store standard re-vertexing configuration
{
    //Tell the task to re-run vertexers
    SetRunV0Vertexer(kTRUE);
...
//________________________________________________________________________
Long_t AliAnalysisTaskWeakDecayVertexer_mod::Tracks2V0vertices(AliESDEvent *event) {
    //--------------------------------------------------------------------
    ...
    int nCheck0=0,nCheck1=0,nCheck2=0,nCheck3=0,nCheck4=0,nCheck5=0,nCheck6=0;
    Long_t i;
    for (i=0; i<nentr; i++) {
    ...
    for (i=0; i<nneg; i++) {
        ...
        for (Int_t k=0; k<npos; k++) {
            ...
            nCheck1++;
            //printf("-> %.2f %.2f %.2f\n",xn+xp,fV0VertexerSels[6],fV0VertexerSels[5]);
...
//            nCheck3++;
    ...
    Info("Tracks2V0vertices: Checks","%d %d %d %d %d %d %d",nCheck0,nCheck1,nCheck2,nCheck3,nCheck4,nCheck5,nCheck6);
...
//________________________________________________________________________
Long_t AliAnalysisTaskWeakDecayVertexer_mod::V0sTracks2CascadeVertices(AliESDEvent *event) {
    //--------------------------------------------------------------------
    ...
    Int_t n0=0,n1=0,n2=0,n3=0,n4=0,n5=0,n6=0,n7=0;
    TObjArray vtcs(nV0);
    Long_t i;
    for (i=0; i<nV0; i++) {
        ...
        //1) DCA V0 Dau
        if( v->GetDcaV0Daughters() > fV0VertexerSels[3] ) {n0++; continue;}
        
        //2) CosPA
        if( v->GetV0CosineOfPointingAngle(xPrimaryVertex,yPrimaryVertex,zPrimaryVertex)< fV0VertexerSels[4] ) {n1++; continue;}
        ...
        Double_t lDcaNegToPrimVertex = TMath::Abs(nTrack->GetD(xPrimaryVertex,
                                                               yPrimaryVertex,
                                                               b) );
        if(lDcaNegToPrimVertex<fV0VertexerSels[1]||lDcaPosToPrimVertex<fV0VertexerSels[2]) {n2++; continue;} //ignore if too close
        ...
        if(lV0Radius<fV0VertexerSels[5]) {n3++; continue;}
        
        //5) kTPC refit check
        // TPC refit condition (done during reconstruction for Offline but not for On-the-fly)
        if( !(pTrack->GetStatus() & AliESDtrack::kTPCrefit)) {n4++; continue;}
        if( !(nTrack->GetStatus() & AliESDtrack::kTPCrefit)) {n5++; continue;}
        ...
        if( (TMath::Abs(lNegEta)>0.8 || TMath::Abs(lPosEta)>0.8)&&fkExtraCleanup ) {n6++; continue;}
        ...
        if (v->GetD(xPrimaryVertex,yPrimaryVertex,zPrimaryVertex)<fCascadeVertexerSels[1]) {n7++; continue;}
        vtcs.AddLast(v);
    }
    printf("V0sTracks2CascadeVertices before sel %d\n", nV0);
    Printf("V0sTracks2CascadeVertices Preselect v0: %d, %d, %d, %d, %d, %d, %d, %d",n0,n1,n2,n3,n4,n5,n6,n7);
    nV0=vtcs.GetEntriesFast();
    printf("V0sTracks2CascadeVertices after sel %d\n", nV0);
    ...
    for (i=0; i<nentr; i++) {
        ...
        if ((status&AliESDtrack::kITSrefit)==0)
            if ((status&AliESDtrack::kTPCrefit)==0) {n8++; continue;}
        ...
        if (TMath::Abs(esdtr->GetD(xPrimaryVertex,yPrimaryVertex,b))<fCascadeVertexerSels[3]) {n9++; continue;}
        trk[ntr++]=i;
    }
    Printf("V0sTracks2CascadeVertices Single Track sel: after track sel %d %d\n", nentr, ntr);
    Printf("V0sTracks2CascadeVertices Single Track sel: rejected        %d %d", n8, n9);
    Double_t massLambda=1.11568;
    Long_t ncasc=0;
    
    Int_t ncheck0=0,ncheck1=0,ncheck2=0,ncheck3=0,ncheck4=0,ncheck5=0,ncheck6=0,ncheck7=0,ncheck8=0,ncheck9=0,ncheck10=0,ncheck11=0,ncheck12=0;
    ...
    for (i=0; i<nV0; i++) { //loop on V0s
        ...
        //printf("%f %f\n",TMath::Abs(v0.GetEffMass()-massLambda),fCascadeVertexerSels[2]);
        ncheck0++;
        for (Int_t j=0; j<ntr; j++) {//loop on tracks
            ...
            //printf("before propagate to DCA %f %f", b, lBachMassForTracking);
            Double_t dca=PropagateToDCA(pv0,pbt,event,b,lBachMassForTracking);
            //printf("before propagate to DCA");
    ...
    Printf("V0sTracks2CascadeVertices checks cascades %d %d %d %d %d %d %d %d %d %d %d %d %d %d",ncheck0,ncheck1,ncheck2,ncheck3,ncheck4,ncheck5,ncheck6,ncheck7,ncheck8,ncheck9,ncheck10,ncheck11,ncheck12,ncasc);
    
    Int_t ncheck00=0,ncheck1_1=0,ncheck22=0,ncheck33=0,ncheck44=0,ncheck55=0,ncheck66=0,ncheck77=0,ncheck88=0,ncheck99=0,ncheck100=0,ncheck111=0,ncheck122=0;
    ...
    for (i=0; i<nV0; i++) { //loop on V0s
        ...
        for (Int_t j=0; j<ntr; j++) {//loop on tracks
            ...
            Double_t x,y,z; cascade.GetXYZcascade(x,y,z); // Bo: bug correction
            Double_t r2=x*x + y*y;
            if (r2 > fCascadeVertexerSels[7]*fCascadeVertexerSels[7]) continue;
            ncheck55++;// condition on fiducial zone
            if (r2 < fCascadeVertexerSels[6]*fCascadeVertexerSels[6]) continue;
            ncheck66++;
    ...
    Printf("V0sTracks2CascadeVertices checks anti-cascades %d %d %d %d %d %d %d %d %d %d %d %d %d %d",ncheck00,ncheck1_1,ncheck22,ncheck33,ncheck44,ncheck55,ncheck66,ncheck77,ncheck88,ncheck99,ncheck100,ncheck111,ncheck122,ncasc);
...
//________________________________________________________________________
Long_t AliAnalysisTaskWeakDecayVertexer_mod::V0sTracks2CascadeVerticesUncheckedCharges(AliESDEvent *event) {
    //--------------------------------------------------------------------
    // This function reconstructs cascade vertices
    //      Adapted to the ESD by I.Belikov (Jouri.Belikov@cern.ch)
...
//________________________________________________________________________
Double_t AliAnalysisTaskWeakDecayVertexer_mod::Det(Double_t a00, Double_t a01, Double_t a10, Double_t a11) const {
    //--------------------------------------------------------------------
...
//________________________________________________________________________
Double_t AliAnalysisTaskWeakDecayVertexer_mod::Det(Double_t a00,Double_t a01,Double_t a02,
...
//________________________________________________________________________
Double_t AliAnalysisTaskWeakDecayVertexer_mod::PropagateToDCA(AliESDv0 *v, AliExternalTrackParam *t, AliESDEvent *event, Double_t b, Double_t lBachMassForTracking) {
    //--------------------------------------------------------------------
...
//________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::Evaluate(const Double_t *h, Double_t t,
...
//________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::CheckChargeV0(AliESDv0 *v0)
{
...
Double_t AliAnalysisTaskWeakDecayVertexer_mod::GetDCAV0Dau( AliExternalTrackParam *pt, AliExternalTrackParam *nt, Double_t &xp, Double_t &xn, Double_t b, Double_t lNegMassForTracking, Double_t lPosMassForTracking) {
    //--------------------------------------------------------------
    ...
    if( fkDoImprovedDCAV0DauPropagation){
        ...
        if( (lDist > TMath::Abs(NegRadius-PosRadius)) && (lDist < NegRadius + PosRadius) && fkXYCase2 ){
            ...
            if( xThisNeg[0] < lMaximumX && xThisPos[0] < lMaximumX && xThisNeg[0] > lMinimumX && xThisPos[0] > lMinimumX ){
                ...
                //for(Int_t ll=0; ll<3; ll++){
                //    Printf("lCase2aNegR[%d]=%f \t lCase2aPosR[%d]=%f",ll,lCase2aNegR[ll],lCase2aPosR[ll]);
                //}
                //Printf("TMath::Power(lCase2aNegR[0]-lCase2aPosR[0],2)=%f TMath::Power(lCase2aNegR[1]-lCase2aPosR[1],2)=%f TMath::Power(lCase2aNegR[2]-lCase2aPosR[2],2)=%f sum=%f",TMath::Power(lCase2aNegR[0]-lCase2aPosR[0],2),
                       //TMath::Power(lCase2aNegR[1]-lCase2aPosR[1],2),
                       //TMath::Power(lCase2aNegR[2]-lCase2aPosR[2],2),
                       //TMath::Power(lCase2aNegR[0]-lCase2aPosR[0],2)+TMath::Power(lCase2aNegR[1]-lCase2aPosR[1],2)+TMath::Power(lCase2aNegR[2]-lCase2aPosR[2],2));
            ...
            if( xThisNeg[1] < lMaximumX && xThisPos[1] < lMaximumX && xThisNeg[1] > lMinimumX && xThisPos[1] > lMinimumX ){
                ...
                //Printf("TMath::Power(lCase2bNegR[0]-lCase2bPosR[0],2)+TMath::Power(lCase2bNegR[1]-lCase2bPosR[1],2)+TMath::Power(lCase2bNegR[2]-lCase2bPosR[2],2)= %f + %f +%f = %f",TMath::Power(lCase2bNegR[0]-lCase2bPosR[0],2),TMath::Power(lCase2bNegR[1]-lCase2bPosR[1],2),TMath::Power(lCase2bNegR[2]-lCase2bPosR[2],2),TMath::Power(lCase2bNegR[0]-lCase2bPosR[0],2)+TMath::Power(lCase2bNegR[1]-lCase2bPosR[1],2)+TMath::Power(lCase2bNegR[2]-lCase2bPosR[2],2));
                if( lPropagA && lPropagB ){
                ...
                //Printf("lCase2bDCA= %f",lCase2bDCA);
                }else{
        ...
        //Printf("Before lPreprocessDCAxy < 999");
        ...
        //Printf("After lPreprocessDCAxy < 999");
        
        //don't redefine!
        //Printf("Before 1 GetHelixParameters");
        nt->GetHelixParameters(p1,b);
        //Printf("After 1 GetHelixParameters");
        p1[6]=TMath::Sin(p1[2]);
        p1[7]=TMath::Cos(p1[2]);
        //Printf("Before 2 GetHelixParameters");
        pt->GetHelixParameters(p2,b);
        //Printf("After 2 GetHelixParameters");
        p2[6]=TMath::Sin(p2[2]);
        p2[7]=TMath::Cos(p2[2]);
        //Printf("p1[6]=%f p1[7]=%f p2[6]=%f p2[7]=%f",p1[6],p1[7],p2[6],p2[7]);
        //+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    }
    ...
    Evaluate(p2,t2,r2,g2,gg2);
    //for(Int_t i=0; i<3; i++){
    //Printf("r1[i]=%f ,g1[i]=%f ,gg1[i]=%f ,t1=%f ",r1[i],g1[i],gg1[i],t1);
    //Printf("r2[i]=%f ,g2[i]=%f ,gg2[i]=%f ,t2=%f ",r2[i],g2[i],gg2[i],t2);
    //}
    Double_t dx=r2[0]-r1[0], dy=r2[1]-r1[1], dz=r2[2]-r1[2];
    Double_t dm=dx*dx/dx2 + dy*dy/dy2 + dz*dz/dz2;
    ...
    //Printf("dy2*dz2=%f dm=%f",dy2*dz2,dm);
    //Printf("TMath::Sqrt(dm*TMath::Sqrt(dy2*dz2))=%f",TMath::Sqrt(dm*TMath::Sqrt(dy2*dz2)));
    
    return TMath::Sqrt(dm*TMath::Sqrt(dy2*dz2));
}

///________________________________________________________________________
void AliAnalysisTaskWeakDecayVertexer_mod::GetHelixCenter(const AliExternalTrackParam *track,Double_t center[2], Double_t b){
